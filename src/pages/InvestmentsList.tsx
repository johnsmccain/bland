...
import { useGetRecentActivities } from '../hooks/contracts/useRecentActivities';
...
export function InvestmentsList() {
  ...
  const { userId} = useContract("1")
  const { isConnected } = useAccount();
  // Pagination state
  const [currentPage, setCurrentPage] = useState(1);
  
  // Fetch recent activities with pagination
  const { data: getRecentActivities } = useGetRecentActivities(userId, BigInt((currentPage)))
